...
export default function WorldStateVoting({ worldId, campaignId }) {
  const [proposalText, setProposalText] = useState("");
  const [proposalType, setProposalType] = useState("world_change");
  const queryClient = useQueryClient();

  const { data: proposals } = useQuery({
    queryKey: ['worldStateProposals', worldId, campaignId],
    queryFn: async () => {
      try {
  ...
  const createProposalMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['worldStateProposals', worldId, campaignId] });
      setProposalText("");
  ...
  const voteProposalMutation = useMutation({
    mutationFn: async ({ proposalId, voteType }) => {
      ...
      if (votesFor.length > votesAgainst.length && votesFor.length >= Math.ceil(totalVotes * 0.6)) {
        status = 'approved';
      } else if (votesAgainst.length > votesFor.length && votesAgainst.length >= Math.ceil(totalVotes * 0.6)) {
        status = 'rejected';
      }
              ...
              <div key={proposal.id} className="bg-slate-700/30 border border-slate-500/30 rounded-lg p-3 space-y-2">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1">
                    {proposal.status === 'approved' && <Badge className="bg-green-600">✓ Approved</Badge>}
                    {proposal.status === 'rejected' && <Badge className="bg-red-600">✗ Rejected</Badge>}
                    {proposal.status === 'active' && <Badge className="bg-yellow-600">Active</Badge>}
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <Progress value={getVotePercentage(proposal.votes_for || [], totalVotes)} className="flex-1" />
                    <span className="text-xs text-slate-400">{proposal.votes_for?.length || 0}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <ThumbsDown className="w-4 h-4 text-red-400" />
                    <Progress value={getVotePercentage(proposal.votes_against || [], totalVotes)} className="flex-1" />
                    <span className="text-xs text-slate-400">{proposal.votes_against?.length || 0}</span>
                  </div>
                </div>
